...
def build_from_path(in_dir, base_dir, out_dir, hparams, num_workers=1, tqdm=lambda x: x):
  ...
  futures_val = []
  index = 1
  with open(os.path.join(in_dir, 'script', 'script.txt'), encoding='utf-8', errors='ignore') as f:  # script_punc_num.txt, encoding='cp949'
    for line in f:
      parts = line.rstrip('\n').split('\t')
      # index = line[1:5]
      #if parts[0][0] != 'f':
      wav_path = os.path.join(in_dir, 'wav', '%s.wav' % parts[0])  # without [0])
...
def write_metadata(metadata, out_dir, domain):
  ...
  print('Min input length:  %d' % min(len(m[3]) for m in metadata))
  print('Min output length: %d' % min(m[2] for m in metadata))