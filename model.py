...
from nlp.distil_model import EmotionDistilBert
...
class Encoder(nn.Module):
    ...
    def forward(self, x, input_lengths):
        ...
        x = x.transpose(1, 2)
        total_length = x.size(1) # This variable is required for DataParallel use
...
class Decoder(nn.Module):
    ...
    def decode(self, decoder_input, attention_weights=None):
        ...
        attention_weights:
        """
        cell_input = torch.cat((decoder_input, self.attention_context), -1) #decoder_input: prev_mel, attention_context: prev_decoder_timestep_context_on_texts
        ...
        self.attention_weights_cum += self.attention_weights #attention weights for this decoder time step
    ...
    def forward(self, memory, decoder_inputs, memory_lengths):
        ...
        # Global batch max_len is required for mask length to make DataParallel possible.
        self.initialize_decoder_states( # initialize decoder member variables for dymanic tensor
            memory, mask=~get_mask_from_lengths(memory_lengths, memory.size(1)))
    ...
    def inference_noattention(self, memory, attention_map):
        ...
        return mel_outputs, gate_outputs, alignments
'''
inputs and labels are loaded on gpu and grouped into input and label
'''
# This used to be model function but it is made static function to be used globally outside the parallelism.
def parse_batch(batch):
    text_padded, input_lengths, mel_padded, gate_padded, \
    output_lengths, speaker_ids, f0_padded, input_ids, attention_mask, speaking_emotion = batch
    ...
    input_ids = to_gpu(input_ids.data).long()
    attention_mask = to_gpu(attention_mask.data).long()
    speaking_emotion = to_gpu(speaking_emotion.data).long()
...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.emotion_embedding = nn.Embedding(hparams.n_emotion_labels, hparams.emotion_embedding_dim)
    ...
    def forward(self, inputs):
        inputs, input_lengths, targets, max_len, \
            output_lengths, speaker_ids, f0s, input_ids, attention_mask, emotion_supervision = inputs
        input_lengths, output_lengths = input_lengths.data, output_lengths.data
        embedded_inputs = self.embedding(inputs).transpose(1, 2)
        embedded_text = self.encoder(embedded_inputs, input_lengths)
        embedded_speakers = self.speaker_embedding(speaker_ids)[:, None]
        embedded_emotions = self.emotion_embedding(emotion_supervision)[:,None]
        len = int(embedded_text.size(1))
        embedded_speakers = embedded_speakers.repeat(1, len, 1)
        embedded_emotions = embedded_emotions.repeat(1, len, 1)
        encoder_outputs = torch.cat(
            (embedded_text, embedded_emotions, embedded_speakers)
        , dim=2)
    ...
    def inference(self, inputs):
        self.emotion_distil_bert = EmotionDistilBert.from_pretrained('/home/admin/projects/graduate/DistilBERT_emotion_classifier/checkpoints/distilBERT/Acc3/checkpoint_30_epochs').cuda().eval()
        inputs, input_lengths, targets, max_len, \
            output_lengths, speaker_ids, f0s, input_ids, attention_mask, emotion_supervision = inputs
        input_lengths, output_lengths = input_lengths.data, output_lengths.data
        embedded_inputs = self.embedding(inputs).transpose(1, 2)
        embedded_text = self.encoder(embedded_inputs, input_lengths)
        embedded_speakers = self.speaker_embedding(speaker_ids)[:, None]
        bert_emotion_embedding, bert_represntation = self.emotion_distil_bert(input_ids, attention_mask=attention_mask)
        # TODO: Add uncertainty prediction
        bert_emotion_embedding = torch.argmax(bert_emotion_embedding)
        # bert_emotion_embedding[:,0]=0.0
        # bert_emotion_embedding[:,1]=0.0
        # bert_emotion_embedding[:,2]=0.0
        # bert_emotion_embedding[:,3]=1.0
        bert_emotion_embedding = self.emotion_embedding(bert_emotion_embedding)[:,None].unsqueeze(1).transpose(0,2)
        len = int(embedded_text.size(1))
        embedded_speakers = embedded_speakers.repeat(1, len, 1)
        embedded_emotions = bert_emotion_embedding.repeat(1, len, 1)
        encoder_outputs = torch.cat(
            (embedded_text, embedded_emotions, embedded_speakers)
        , dim=2)
    ...
    def inference_override(self, inputs, emotion_idx):
        self.emotion_distil_bert = EmotionDistilBert.from_pretrained(
            '/home/admin/projects/graduate/DistilBERT_emotion_classifier/checkpoints/distilBERT/Acc3/checkpoint_30_epochs').cuda().eval()
        inputs, input_lengths, targets, max_len, \
        output_lengths, speaker_ids, f0s, input_ids, attention_mask, emotion_supervision = inputs
        input_lengths, output_lengths = input_lengths.data, output_lengths.data
        embedded_inputs = self.embedding(inputs).transpose(1, 2)
        ...
        # bert_emotion_embedding[:,2]=0.0
        # bert_emotion_embedding[:,3]=1.0
        bert_emotion_embedding = self.emotion_embedding(to_gpu(emotion_idx))[:, None]
        len = int(embedded_text.size(1))
        embedded_speakers = embedded_speakers.repeat(1, len, 1)
        embedded_emotions = bert_emotion_embedding.repeat(1, len, 1)
        encoder_outputs = torch.cat(
